...
from ..hamlib.responses import (
    ...
    ModeResponse, VfoResponse, PttResponse, PowerstatResponse,
    ChkVfoResponse, DumpStateResponse, DumpCapsResponse,
    LevelResponse, SplitVfoResponse
)
...
class RigClient:
    ...
    async def execute(self, cmd: HamlibCommand) -> HamlibResponse:
        """Execute a HamlibCommand and return a HamlibResponse.
        
        This is the unified entry point for the router to dispatch commands.
        Delegates to the appropriate method based on command type.
        
        Args:
            cmd: The command to execute.
            
        Returns:
            Appropriate HamlibResponse subclass.
        """
        try:
            # SET COMMANDS
            if isinstance(cmd, SetFreq):
                ok = await self.set_frequency(cmd.frequency)
                return SuccessResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
                    result=0 if ok else -11,
                )
            
            if isinstance(cmd, SetMode):
                ok = await self.set_mode(cmd.mode, cmd.passband)
                return SuccessResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
            ...
            if isinstance(cmd, SetVfo):
                ok = await self.set_vfo(cmd.vfo)
                return SuccessResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
            ...
            if isinstance(cmd, SetPtt):
                ok = await self.set_ptt(cmd.ptt)
                return SuccessResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
            ...
            if isinstance(cmd, GetPowerstat):
                stat = await self.get_powerstat()
                if stat is None:
                    return BaseResponse(
                        cmd=cmd.cmd,
                        request_id=cmd.request_id,
            ...
            if isinstance(cmd, DumpState):
                lines = await self.dump_state()
                return DumpStateResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
                    lines=list(lines),
                    result=0 if lines else -11,
                )
            
            if isinstance(cmd, DumpCaps):
                lines = await self.dump_caps()
                return DumpCapsResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
                    lines=list(lines),
            ...
            if isinstance(cmd, GetLevel):
                # Stub: return 0 for now (matches current behavior)
            ...
            if isinstance(cmd, GetSplitVfo):
                vfo = await self.get_vfo()
                return SplitVfoResponse(
                    cmd=cmd.cmd,
                    request_id=cmd.request_id,
                    source=cmd.source,
                    split=0,
            ...
            # Unknown command
            return BaseResponse(
                cmd=getattr(cmd, 'cmd', 'unknown'),
                request_id=cmd.request_id,
                source=cmd.source,
                result=-4,  # Unknown command
            )
