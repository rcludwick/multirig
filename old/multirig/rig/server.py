...
from ..hamlib.parser import parse_line
from ..hamlib.formatter import format_response
from ..hamlib.messages import GetInfo, Model, Version, Token, SetConf, GetConf
from ..hamlib.responses import (
    InfoResponse, ModelResponse, VersionResponse, ConfResponse, SuccessResponse
)
...
class RigctlServer(BaseTcpServer):
    ...
    async def _handle_command_line(self, line: str) -> bytes:
        ...
        # If router is available, use the new unified path
        if self._router is not None:
            return await self._handle_via_router(line, erp_prefix)

        # Fall back to legacy command dispatch
        parts = cmdline.split()
        cmd = parts[0]
        args = parts[1:]
    ...
    async def _handle_via_router(self, line: str, erp_prefix: Optional[str]) -> bytes:
        """Handle command using the router (new unified path)."""
        # Parse the command
        cmd = parse_line(line, source="rigctl")
        if cmd is None:
            return self._format_error("parse", erp_prefix, -1)
        
        # Handle static/special commands locally
        if isinstance(cmd, GetInfo):
            resp = InfoResponse(
                cmd=cmd.cmd,
            ...
            return format_response(resp, erp_prefix)
        ...
        # Determine if we should broadcast (set commands with sync enabled)
        broadcast = False
        if self.get_rigctl_to_main_enabled() and self.get_sync_enabled():
            from ..hamlib.messages import SetFreq, SetMode, SetPtt
            if isinstance(cmd, (SetFreq, SetMode, SetPtt)):
                broadcast = True
        
        # Submit to router
        resp = await self._router.submit(cmd, broadcast=broadcast)
