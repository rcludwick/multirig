...
def test_sync_to_follower_band_error(page: Page, profile_manager: ProfileManager):
    ...
    try:
        ...
        # Status update is async, so we need to poll
        expect(page).to_have_url(re.compile(".*")) # Ensure we have expect available if needed, though we use python retry logic here
        ...
        for _ in range(10): # Try for 2 seconds (10 * 0.2s)
            ...
            assert res.ok
            status = res.json()
            err = status["rigs"][1]["last_error"]
            if err is not None and "Frequency out of configured band ranges" in err:
                last_error_found = True
                break
            time.sleep(0.2)
            
        assert last_error_found, f"Expected error not found in status. Last status error: {status['rigs'][1]['last_error']}"
